...
.then(function(data) {
  ...
  const info = data.imagenes1
  const info2 = data.imagenes2
...
videoButton.addEventListener('click', function(e) {
  ...
  .then(function(data) {
    ...
    createVideo.src = urlWeb + 'videos/' + video.path ;
...
.then(function(data){
  ...
  videos.forEach((video) => {
    ...
    createVideo.setAttribute('poster', `${urlWeb}img/${video.poster}`)
  ...
  function putVideo(){
    ...
    videos.forEach((video) => {
      ...
      if(newStr == video.id){
        ...
        createSource.src = urlWeb + 'videos/' + video.path + '.mp4';
